...
class NutritionAlgorithm {
  ...
  late final (double, double, double, double) _modeWeights = modeWeightsCalc(
    mode,
  );
  late final double _activityMultiplier = activityLevelMultiplier(
    activityLevel,
  );
  ...
  int caloriesNeeded(int exerciseMinutes) {
    ...
    num caloriesBurnedFromExercise =
        exerciseMinutes /
        30 *
        200; //assuming 200 calories burned per 30 minutes of exercise

    return (BMR * _activityMultiplier * _modeWeights.$1 +
  ...
  num waterNeeded(int exerciseMinutes) {
    return (weight * 35 + (1080 + 240 * exerciseMinutes / 10)) /
        1000; //ml to liters. 35ml per kg of body weight + 240ml per 10 minutes of exercise + 240ml for daily activities
  }
}
